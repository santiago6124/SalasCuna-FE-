...
export default function DropdownCribroomList(authTokens) {
  const [cribrooms, setCribrooms] = useState([]);
  const [childs, setChild] = useState([]);
  const [selectedCribroom, setSelectedCribroom] = useState('');
  const [showNewButton, setShowNewButton] = useState(false);

  console.log('authTokens: ', authTokens.authTokens);
  ...
  async function handleCargarClick() {
    if (selectedCribroom) {
      try {
        ...
        const res = await child_request(authTokens.authTokens, 'get', 0, {}, undefined, '&cribroom_id=' + selectedCribroom);

        console.log('Child API Response:', res.data);
  ...
  async function handleLoadCribroomOptions(inputValue) {
    try {
      ...
      const response = await cribroom_request(authTokens.authTokens, 'get', 0, {}, undefined, '');
